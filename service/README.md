...
For Docker on the selected server, supply `RELEASE_HOST` (hostname only) and `OWNER_KEY_PATH` (private key-file path). Make the key readable by the container service user through a protected Docker secret mount; verify host file permissions. For this image the service UID is 1000: on Linux, the secret file should be owned by UID 1000 with mode 600 so the non-root service can read it. Start with:
...
The real-package integration probe is `node service/tests/real-package.mjs PATH-TO-PRIVATE-MANIFEST`, with `TEST_TLS_CERT` and `TEST_TLS_KEY` pointing to the isolated certificate files generated by the test script. It publishes and activates a real package on an ephemeral loopback TLS service, downloads all authenticated bytes and verifies SHA-256, then revokes the tester and removes the temporary database/packages. No internet listener is created. The Warplex AE 0.1.1-preview package passed this probe (65,661,490 bytes).
